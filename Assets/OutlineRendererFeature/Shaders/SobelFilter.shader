Shader "Unlit/SobelFilter"
{
	Properties 
	{
		...
		_Delta ("Line Thickness", Range(0.0005, 0.0035)) = 0.001
		_AlphaStep ("Alpha Step", Range(0, 1)) = 0.15
		_AlphaStepWidth ("Alpha Step Width", Range(0, 1)) = 0.01
	...
	SubShader 
	{
		...
		Pass
        {
                    ...
                    {
        #if defined(UNITY_STEREO_INSTANCING_ENABLED) || defined(UNITY_STEREO_MULTIVIEW_ENABLED)
                        return SAMPLE_TEXTURE2D_ARRAY(_CameraColorTexture, sampler_CameraColorTexture, uv, unity_StereoEyeIndex).a;
        #else
                    	return smoothstep(_AlphaStep, _AlphaStep + _AlphaStepWidth, SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, uv).a);
        #endif
                    }
                    ...
                    float sobel (float2 uv) 
                    {
                        ...
                        hr += SampleAlpha(uv + float2(-1.0, -1.0) * delta) *  1.0;
                        hr += SampleAlpha(uv + float2( 1.0, -1.0) * delta) * -1.0;
                        hr += SampleAlpha(uv + float2(-1.0,  0.0) * delta) *  2.0;
                        hr += SampleAlpha(uv + float2( 1.0,  0.0) * delta) * -2.0;
                        hr += SampleAlpha(uv + float2(-1.0,  1.0) * delta) *  1.0;
                        hr += SampleAlpha(uv + float2( 1.0,  1.0) * delta) * -1.0;
                        
                        vt += SampleAlpha(uv + float2(-1.0, -1.0) * delta) *  1.0;
                        vt += SampleAlpha(uv + float2( 0.0, -1.0) * delta) *  2.0;
                        vt += SampleAlpha(uv + float2( 1.0, -1.0) * delta) *  1.0;
                        vt += SampleAlpha(uv + float2(-1.0,  1.0) * delta) * -1.0;
                        vt += SampleAlpha(uv + float2( 0.0,  1.0) * delta) * -2.0;
                        vt += SampleAlpha(uv + float2( 1.0,  1.0) * delta) * -1.0;
                    ...
                    half4 frag (Varyings input) : SV_Target 
                    {
                    	...
                    	float f = saturate(1 - s);
                    	float steppedA = step(0.01, col.a);
                    	col.rgba = steppedA;
                        return col * f; //(1 - s);
        #endif
                    }
